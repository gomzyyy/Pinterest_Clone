...
export default function ArchivedPosts() {
  ...
  const ArchivedPostPage = (): React.JSX.Element => {
    return (
      <>
              ...
              <FlatList
                ...
                renderItem={({ item, index }) => (
                  <AdminPost
                    i={item}
                    a={item.admin}
                    lastPostMargin={index === posts.length - 1 ? 10 : 0}
                  />
                )}
              />
            </View>
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      >
        ...
        <Text style={{ fontSize: 24, color: colors.col.white }}>Archived</Text>
      </View>
      {error ? <ErrorPage /> : <ArchivedPostPage />}
    </View>
  );
}
